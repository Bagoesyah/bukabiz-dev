...
function ButtonWide({ variant, featured, onClick, icon }) {
  return (
    ...
    >
      <button
        onClick={onClick}
        className={`w-80 py-3  capitalize  duration-300 rounded flex items-center justify-center space-x-3
        ${
          variant === "popup"
            ? "bg-primary text-black text-xs border-black hover:text-white "
            : variant === "warning"
            ? " bg-primary font-semibold text-black border-primary hover:bg-opacity-60"
            : " bg-[#6B6B6B] text-white font-normal hover:bg-opacity-60"
        } `}
      >
        <span>
          {featured === "video"
            ? "Pelajari Detailnya Lewat Video"
            : featured === "image"
            ? "Pelajari Detailnya Lebih Lanjut"
            : "Buka lebih banyak"}
        </span>
